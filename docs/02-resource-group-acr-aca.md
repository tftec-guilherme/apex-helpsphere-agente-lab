...
> **Nota pedagógica — por que ACR Basic e não Standard/Premium?** Basic (R$ 35/mês, 10 GiB storage, 1 webhook) é suficiente para o lab: vamos pushar 2 imagens (`mcp-server` + `n8n` custom), nenhuma com mais de 500 MiB. Standard (R$ 100/mês, 100 GiB, 10 webhooks) só justifica quando você tem geo-replication, ≥3 webhooks, ou content trust (image signing). Premium (R$ 500+/mês) é Private Link + Customer-Managed Keys + tasks paralelas — produção crítica. **Para o lab, Basic. Em produção corporate, suba para Standard como default e Premium só quando compliance exigir.**
...
> **Custo:** ACR Basic = **R$ 35/mês fixo** (cobra parado, não tem scale-to-zero). É a SKU mais barata. Standard alternativa quando justifica upgrade (geo-replication, ≥3 webhooks, content trust): R$ 100/mês. No lab, **fique no Basic** — delete o RG no Cap 09 para não acumular cobrança.
...
- ⚠️ **ACR Basic tem limit de 10 GiB de storage** — se você pushar imagens grandes (ex.: n8n custom com 800 MiB) ou esquecer de purge de tags antigas, atinge cap rapidamente. Sintoma: `denied: requested access to the resource is denied` no `docker push`. Workaround: `az acr repository delete --name <acr> --image <repo>:<tag>` para liberar espaço, ou subir para Standard (R$ 100/mês, 100 GiB). **Em produção, sempre cravar política de retention de tags (`az acr config retention`).**
