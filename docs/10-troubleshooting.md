...
| N5 | n8n node AMQP Trigger ("Service Bus Trigger") polling falha silently com Topic | Sink incompleto — Topic exige formato `<topic>/Subscriptions/<subscription>` (Queue aceita so o nome) | Preencher com `tickets-escalated/Subscriptions/n8n-escalation-sub` (ver [`_disclaimers.md`](./_disclaimers.md) **AMB-4**) | Cap 07 |
...
| N7 | n8n nao tem Service Bus node first-party (`azureServiceBusTrigger` nunca existiu) — workflow falha import com "This node is not currently installed" | Catalogo `n8n.io/integrations` nao lista; pasta `nodes-base/nodes/Microsoft/` no GitHub tampouco. Issue n8n-io/n8n#12959 confirma | Usar `n8n-nodes-base.amqpTrigger` (AMQP 1.0 first-party) + credential AMQP 1.0. Service Bus suporta AMQP nativo. Tradeoff: sem peek-lock/DLQ programatica do SDK | Cap 07 |
...
| S2 | Workflow JSON `escalation-servicebus-sheets.json` usa `topic: escalations` mas Caps 07/08 cravam `tickets-escalated` | Scaffold inicial desalinhado dos passos finais | Editar node AMQP Trigger ("Service Bus Trigger") no n8n UI direto (sobrescreve JSON); **gap follow-up prof:** atualizar JSON canônico do scaffold (Story 06.32 ja atualizou para AMQP Trigger v0.3.0-amqp) | Cap 08 + §7 |
