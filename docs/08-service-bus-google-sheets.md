...
>
> Cada subscription tem **lock-duration próprio** + **dead-letter próprio** — falha em uma não afeta a outra. Amanhã, adicionar **`sub-bi`** para uma Function App escrever em SQL warehouse é gratuito e zero código no agente.

> **Nota pedagógica — Connection String vs Managed Identity, qual usar?** No capítulo anterior cravamos a credential do n8n em **rascunho** com Connection String porque a MI ainda não tinha role assignment. Vamos **manter Connection String no n8n** porque o trigger `Azure Service Bus` ainda **não suporta MI nativamente** (limitação do node oficial — issue aberto há ~2 anos). **No agente Foundry usamos MI** porque o SDK Python `azure-servicebus` suporta `DefaultAzureCredential` end-to-end. **Em produção real:** rodar o consumer Service Bus em uma Function App ou Container App com MI, ou esperar o node n8n suportar MI.
