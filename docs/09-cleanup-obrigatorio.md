...
> **Atenção custo recorrente — POR QUE este capítulo é obrigatório, não opcional:** o Lab Final introduz **3 recursos que cobram parados** no `rg-lab-final`: **PostgreSQL Burstable B1ms** (~R$ 60/mês ligado 24×7 ou ~R$ 5/mês idle storage se Stop), **ACR Basic** (~R$ 35/mês fixo, cobra mesmo sem imagens), **Service Bus Standard** (~R$ 50/mês baseline porque AMB-4 forçou Standard para ter Topics — Basic não tem). Soma: **~R$ 145/mês "esquecidos"**. Se você executar o lab e não fazer cleanup, **em 30 dias o cartão é debitado de R$ 145** sem nenhum tráfego acontecendo. Em sub Free Trial $200 USD: queima crédito por nada e quando os $200 acabam, recursos PARAM e o aluno não entende o motivo.
...
- ⚠️ **PostgreSQL Burstable `Stopped` ainda cobra storage idle E reinicia sozinho em 7d** — Stop zera compute mas storage 32 GiB continua faturando ~R$ 5/mês, e dia 8 Azure liga o server automaticamente cobrando R$ 60/mês 24×7 sem aviso. Workaround: para **0 R$ permanente**, **delete** do RG (este Cap) — Cap 07 Passo 7.7 Stop é só para sessão recorrente curta. Sempre crave **Cost Anomaly Alert R$ 50** (Passo 9.7).
- ⚠️ **Service Bus Standard cobra baseline R$ 50/mês mesmo sem mensagens** — diferente de Basic (R$ 0,05/1M ops), Standard tem **fee fixo** porque você paga pela feature Topics/Subscriptions (decisão AMB-4). Workaround: delete do RG no Passo 9.2 zera; confira Cost Analysis para namespace órfã em RG diferente (typo no Cap 08).
